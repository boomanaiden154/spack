...
class Xpmem(AutotoolsPackage):
    ...
    conflicts("~kernel-module", when="@:2.6.5", msg="Xpmem: cannot disable kernel module with versions prior to 2.6.5.")

    # Ideally, we should list all non-Linux-based platforms here:
    conflicts("+kernel-module", when="platform=darwin", msg="Xpmem: cannot build kernel module on non-Linux platforms.")
