...
class Knem(AutotoolsPackage):
    ...
    conflicts("+hwloc", when="@:0.9.0", msg="Knem: Cannot build with hwloc with versions lower than 0.0.0")
    ...
    for __compiler in spack.compilers.supported_compilers():
        if __compiler != "gcc":
            conflicts(
                "%{0}".format(__compiler), msg="Knem: Linux kernel module must be compiled with gcc"
            )
